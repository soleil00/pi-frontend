...
export default function Shop() {
  ...
  const signInUser = async(authResult: AuthResult) => {
    ...
    const newUser = JSON.stringify(response.data.currentUser)
    const username = JSON.stringify(response.data.currentUser.username)
    
    console.log("this is token : "+ token)
    console.log("this is currentUser : " + newUser)
    console.log("this is currentUser username : " + username)
...
const onReadyForServerApproval = (paymentId: string) => {
  ...
  if (token !== "") {
    console.log("here is token im using : " + token)
    axiosClient.post('/payments/approve', { paymentId }, {
    headers: {
      Authorization: `Bearer ${token}`, // Assuming 'token' is your bearer token
  ...
  return (
    <>
      ...
      <div>here is your token { token}</div>
    </>
  );
}
